...
class Log:
    ...
    @staticmethod
    def from_link(link: str, timeout: int = 4):
        paste_ee_match = re.search(r"https://(?:api\.)?(?:pastee\.dev|paste\.ee)/(?:p/|d/)([a-zA-Z0-9]+)", link)
        mclogs_match = re.search(r"https://(?:api\.)?mclo\.gs/(?:1/raw/)?(\w+)", link)
    ...
    def type(self) -> LogType | None:
        ...
        if any(self.has_content(launcher_log) for launcher_log in [
            " D | ",                          # prism 9.4-
            "Info: Prism Launcher,",          # prism 10.0+
            " D \"MultiMC 5\" ",              # multimc
    ...
    def stacktrace_hash(self) -> str | None:
        ...
        if stacktrace_num == 0:
            ...
            if not match is None:
                trimmed_stacktrace = match.group(1)
        elif stacktrace_num == 9:
            pattern = r"(?m)^encountered an unexpected exception.*\r?\n([^\r\n]*\r?\n[^\r\n]*)"
            match = re.search(pattern, stacktrace)
            if not match is None:
                trimmed_stacktrace = match.group(1)
    ...
    def exitcode(self) -> int | None:
        ...
        matches = list(re.finditer(pattern, self._content, re.DOTALL))
        if matches:
            match = matches[-1]
